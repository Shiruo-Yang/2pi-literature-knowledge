...
def update_formulations(rows: list[dict[str, str]], identities: dict[str, dict[str, Any]]) -> tuple[list[dict[str, Any]], int, int]:
    ...
    for original in rows:
        ...
        if ids:
            ...
            row["identity_enrichment_status"]="source_local_identity_crosschecked_current"; row["identity_resolution"]="accepted_source_local_structure_identity"
...
def main() -> int:
    ...
    accepted_path = next((args.base_dir / name for name in ("accepted_source_identity_registry.csv", "accepted_source_identity_registry_previous.csv") if (args.base_dir / name).is_file()), None)
    material_path = next((args.base_dir / name for name in ("multicomponent_material_registry.csv", "multicomponent_material_registry_previous.csv") if (args.base_dir / name).is_file()), None)
    if accepted_path is None or material_path is None:
        raise FileNotFoundError("base directory must contain the accepted identity and material registries")
    oldacc=read_csv(accepted_path); accepted=oldacc+list(identities.values())
    oldmat=read_csv(material_path); material_rows=oldmat+list(materials.values())
    ...
    write_csv(args.output_dir/"source_local_entity_candidates_enriched.csv",ents,list(ents[0].keys())); write_csv(args.output_dir/"evidence_entity_links_enriched.csv",links,list(links[0].keys())); write_csv(args.output_dir/"formulation_linkage_registry_enriched.csv",forms,list(forms[0].keys())); write_csv(args.output_dir/"accepted_source_identity_registry.csv",accepted,ACCEPTED_COLUMNS); write_csv(args.output_dir/"multicomponent_material_registry.csv",material_rows,MATERIAL_COLUMNS); write_csv(args.output_dir/"terminal_nonchemical_registry.csv",terminal_rows,TERMINAL_COLUMNS); write_csv(args.output_dir/"terminal_identity_registry.csv",terminal_identity_rows,TERMINAL_COLUMNS)
    write_sqlite(args.output_dir/"condition_identity_linkage.sqlite",[("source_local_entity_enriched",ents,list(ents[0].keys())),("evidence_entity_link_enriched",links,list(links[0].keys())),("formulation_linkage_enriched",forms,list(forms[0].keys())),("accepted_source_identity",accepted,ACCEPTED_COLUMNS),("multicomponent_material",material_rows,MATERIAL_COLUMNS),("terminal_nonchemical",terminal_rows,TERMINAL_COLUMNS),("terminal_identity",terminal_identity_rows,TERMINAL_COLUMNS)])
